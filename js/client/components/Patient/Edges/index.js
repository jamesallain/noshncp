...
class Edges extends Component {
  ...
  load = () => {
    if (!this.state.loading &&
      this.props.viewer.patient.pageInfo.hasNextPage &&
      ($(window).scrollTop() + $(window).height()) > $(document).height() - 100) {
      return new Promise((resolve) => {
        return this.setState({loading: true}, () => {
          return this.props.relay.setVariables(
            {after: this.props.viewer.patient.pageInfo.endCursor},
            (readyState) => {
  ...
  variablesFirstSet = (props) => {
    const edgeCount = props.viewer.patient.edges.length;
  ...
  edgesRender() {
    return (this.props.viewer.patient.edges.length > 0) ?
      this.props.viewer.patient.edges.reduce((memo, edge) => {
...
export default createContainer(Edges, {
  ...
  fragments: {
    viewer() {
      return Relay.QL`
        fragment on Viewer {
          patient(searchTerm: $searchTerm, first: $first, after: $after) {
            edges {
              node {
                id,
                fullName,
